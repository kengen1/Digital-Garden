  ...
  const ctx = await esbuild.context({
    ...
    plugins: [
      sassPlugin({
        type: "css-text",
        cssImports: true,
        loadPaths: [path.resolve(cwd, "quartz/styles")], // ✅ Ensures correct SCSS path resolution
      }),
      sassPlugin({
        filter: /\.inline\.scss$/,
        type: "css",
        cssImports: true,
        loadPaths: [path.resolve(cwd, "quartz/styles")], // ✅ Adds SASS load paths
      }),
      {
        ...
        setup(build) {
          build.onLoad({ filter: /\.inline\.(ts|js)$/ }, async (args) => {
            ...
            // Remove default exports inserted by ESBuild
